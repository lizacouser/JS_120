...
class TTTGame {
  ...
  // someoneWon() {
  //   return this.isWinner(this.activePlayer);
  // }
  ...
  isMatchWinner() {
    //return player.getScore() === TTTGame.GAMES_TO_WIN;
    return this.activePlayer.getScore() === TTTGame.GAMES_TO_WIN;
  }

  // getMatchWinner() {
  //   if (!this.isWinner()) return null;

  //   if (this.activePlayer.getScore() === TTTGame.GAMES_TO_WIN) {
...
P:
-first person to win 3 wins the match
-keep track of who wins each match
-test to see if anyone's score is 3
-report score after each game
Rules:
  -end game after match
  -no global or static variables


E:
-computer wins 2, I win 3, 3 ties,
-score: 2, 3, You win the match!
-goodbye message

D:
-store score as property in human and computer?
-matchWinner method in game class
-displayScore is part of the game class

A:
TRACKING SCORE
// add property score to human + computer and initialize to 0
// add increment score method to player
// add getScore method to player
// after each round, update score of winner
  // add getWinner() method
  // if winner, update score of winner

WINNING MATCH
//add getMatchWinner method to TTTGame
  //getScore of most recent winner
  //if it's 3, return that player
-if there is a matchWinner, break
  -display MatchWinner
  -end game
-if winner, getMatchWinner
